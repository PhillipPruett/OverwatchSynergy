...
namespace OverwatchSynergy.Tests
{
    ...
    public class ObjectiveWeightTests
    {
        ...
        {
            var overallscores = Calculator.GetOverallScoresForAllHeroes(new[] {new Mercy()} , new[] {new Reinhardt()}, new Control());
        ...
        {
            var attackScores = Calculator.GetOverallScoresForAllHeroes(new[] { new Mercy() }, new[] { new Reinhardt() }, new AssaultAttack());


            var neutralScores = Calculator.GetOverallScoresForAllHeroes(new[] { new Mercy() }, new[] { new Reinhardt() }, new Control());
